...
if __name__ == "__main__":
    ...
    parser.add_argument("dir", help="frame directory for miou.")
    args = parser.parse_args()
    fdir_path = Path(args.dir)

    a = MeanIoU(100)
    frames = sorted(Path(fdir_path).iterdir(), key=lambda img: int(img.stem))
    miou = []
    for i, file in enumerate(frames[:-1]):
        img1 = Image.open(file)
        img1 = np.array(img1)
        img2 = Image.open(frames[i+1])
        img2 = np.array(img2)
        miou.append(a._mean_iou(img1, img2))
    print(np.mean(miou))
